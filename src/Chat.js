...
const Chat = () => {
  ...
  const sendMessage = async () => {
    await axios.post(`/messages/${roomIds}`, {
             ...
             <>
              ...
              {(format(message.createdAt)).toString()}
               ...
               <div className="chat__footer" style={{position:"relative"}}>
                 {showEmoji === true && (<div style={{position:"absolute",bottom:"80px",left:"10px"}}><Picker onEmojiClick={onEmojiClick} /></div>)}
                 <InsertEmoticonIcon style={{cursor:"pointer"}} onClick={()=> setShowEmoji(prev => !prev)} />
